...
class secondFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        val items = List(20) { index -> "아이템 ${index + 1}" }
        recycler.adapter = SecondAdapter(items)

        recycler.addItemDecoration(DividerItemDecoration(requireContext(), LinearLayoutManager.VERTICAL))
        val spacePx = (8 * resources.displayMetrics.density).toInt()
        recycler.addItemDecoration(SpaceItemDecoration(spacePx))
    }
